...
const string = process.env.API_URL
const previousUrlImages = string.slice(0, -6); // Remove os últimos 4 caracteres
    ...
    const postsList = data.map(item => {
      return `
            <article data-description="${item.descricao}">
              <figure>
                <img src="${previousUrlImages}/${item._id}.png" alt="${item.alt}" />
              </figure>
            </article>
          `
    }).join('');
